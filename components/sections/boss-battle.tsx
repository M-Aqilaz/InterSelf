...
export function BossBattlePanel({ productivityCompletion = 0 }: { productivityCompletion?: number }) {
  ...
  const defeated = currentHp <= 0 && boss !== null;
  ...
  const strike = useCallback(async (s: Strike) => {
    ...
    if (energy <= 0) { addLog("No energy! Wait for regen (1 per hour).", "warning"); return; }
      ...
      <div style={{
        ...
        <div style={{ position: "relative", zIndex: 2, padding: "18px 22px 0", display: "flex", justifyContent: "space-between" }}>
          <div>
            ...
            <div style={{ fontSize: 11, color: "rgba(255,255,255,0.4)", marginTop: 2 }}>Level {boss.level ?? "?"} Boss{boss.weakness ? ` Â· Weak: ${boss.weakness}` : ""}</div>
          </div>
              ...
              return (
                <button key={s.id} type="button" disabled={disabled}
                  ...
                  <div>
                    ...
                    <div style={{ fontSize: 9, color: "rgba(255,255,255,0.35)" }}>{s.desc} Â· CD {s.cooldown}s</div>
                  </div>
